...
WRITE LIKE ${/^[aeiou]/i.test(lang) ? 'AN' : 'A'} ${lang.toUpperCase()} NOVELIST, NOT A TRANSLATOR — the meaning must be exact, but the sentence should read as if it were composed in ${lang} from the start, not carried over from the source language's grammar:
- Break up long source-language sentences built from chained clauses into ${lang} sentences with the rhythm ${lang} readers actually expect. A sentence that runs on in the source because that language's grammar allows it should not automatically run on in ${lang}.
- When the source uses a metaphor or idiom, reach for the closest natural ${lang} image or phrasing that carries the same feeling — not a word-for-word rendering that a ${lang} reader would find odd or literal-sounding, unless the literal image is itself the point.
